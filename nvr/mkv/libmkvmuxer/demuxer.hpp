...
public:
	...
	public:
		Frame()
			: pStream(NULL), isKey(false), timecode(0ull), timecodeEnd(0ull),
			  size(0ul), data(NULL)
		{
		}
		virtual ~Frame() {}
	...
	public:
		...
		Stream(CodecType type = CODEC_TYPE_DEFAULT)
			: pNext(NULL), codecType(type), language(NULL),
			  codec(0), codecPrivateSize(0), pCodecPrivate(NULL)
		{
		}

		virtual ~Stream()
		{
			if (pCodecPrivate != NULL)
			{
				delete[] pCodecPrivate;
			}
		}
		...
		const unsigned char *pCodecPrivate;
	};
