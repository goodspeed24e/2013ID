...
int MkvDemuxer::TranslateCodecIdentifier(const char *codecIdentifier, const Stream * pStream)
{
	...
	default:
		...
		{
			return AudioStream::CODEC_ID_ADPCM;
		}
		...
		{
			return AudioStream::CODEC_ID_G711_ALAW;
		}
...
void MkvDemuxer::MyVideoStream::FixFrameData(Frame &) const
{
}
					...
					if (CHECK_TYPE(pElementLevel2, KaxTrackEntry))
					{
						...
						PARSING_LOOP (pElementLevel2, pElementLevel3, pElementLevel4, pRawdata, relativeUpperLevel)
						{
							...
							else if (CHECK_TYPE(pElementLevel3, KaxTrackLanguage))
							{
								...
								pTrackLanguage = ((std::string)*pLanguage).c_str();  // FIXME: bad life cycle
								MESSAGE("\tlanguage: %s\n", pTrackLanguage);
							}
							else if (CHECK_TYPE(pElementLevel3, KaxCodecID))
							{
								...
								const char *pTrackCodecIdentifier = ((std::string)*pCodecID).c_str();
								trackCodecId = TranslateCodecIdentifier(pTrackCodecIdentifier);
								MESSAGE("\tCodec: (%s), %d\n", pTrackCodecIdentifier, strlen(pTrackCodecIdentifier));
							}
							else
							{
								// WARNING: skip KaxCodecPrivate, KaxContentEncodings, KaxTrackVideo, KaxTrackAudio
								// do nothing
							}
			...
			if (CHECK_TYPE((*pCluster)[0], KaxClusterTimecode))
			{
				...
				for (clusterIndex = 1; clusterIndex < pCluster->ListSize(); clusterIndex++)
				{
					if (CHECK_TYPE((*pCluster)[clusterIndex], KaxSimpleBlock))
					{
						...
						pFrame->pStream  = (pSimpleBlock->TrackNum() == streams.pAudio->trackNumber) ? streams.pAudio
						                   : (pSimpleBlock->TrackNum() == streams.pVideo->trackNumber) ? streams.pVideo
										   : streams.pOther;  // FIXME:
					...
					else if (CHECK_TYPE((*pCluster)[clusterIndex], KaxBlockGroup))
					{
						...
						pFrame->pStream  = (pBlock->TrackNum() == streams.pAudio->trackNumber) ? streams.pAudio
						                   : (pBlock->TrackNum() == streams.pVideo->trackNumber) ? streams.pVideo
										   : streams.pOther;  // FIXME:
						...
						CLUSTER_MESSAGE("\tblock: key=%d, size=%d, timecode=%llx.%llu\n", pFrame->isKey, pFrame->size, pFrame->timecode/1000000000ull, pFrame->timecode/1000000ull%1000ull);
						return pFrame;
					}
					else
					{
