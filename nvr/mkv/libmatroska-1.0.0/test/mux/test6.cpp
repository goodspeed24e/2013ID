...
// older version, write at the end		AllCues.Render(out_file);
		...
		*static_cast<EbmlUInteger *>(&TrackUID) = 13;
		...
		*static_cast<EbmlUInteger *>(&ChapterUID) = 0x67890;
		...
		KaxTagName & Subject = GetChild<KaxTagName>(TagSimple);
		*static_cast<EbmlUnicodeString *>(&Subject) = L"Test?23";

		//KaxTagBibliography & Biblio = GetChild<KaxTagBibliography>(TagGeneral);
		KaxTagString & Biblio = GetChild<KaxTagString>(TagSimple);
		*static_cast<EbmlUnicodeString *>(&Biblio) = L"Áa marche";
		...
		KaxTagLanguage & Lang = GetChild<KaxTagLanguage>(TagSimple);
